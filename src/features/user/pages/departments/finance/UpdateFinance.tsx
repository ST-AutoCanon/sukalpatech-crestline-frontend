...
const UpdateFinance: React.FC = () => {
  const { user, token } = useContext(AuthContext);
  const { allPRs, fetchAllPRs, addComment, addStatus } = useFinance(token);
  ...
  const financeAlreadyActed =
    selectedPR?.comments?.some((c: any) => c.department === "Finance") ||
    selectedPR?.statusLogs?.some((s: any) => s.department === "Finance");
  ...
  const handleSubmit = async () => {
    ...
    if (
      !window.confirm(
        "Are you sure you want to submit Finance comment and status?"
      )
    )
      return;
    ...
    try {
      ...
      await addStatus({
        ...
        updated_by: user.id,
        department: "Finance",
        note: "Finance update via frontend",
      });
        ...
        <>
          ...
          <div>
            ...
            >
              ...
              <option value="Finance Pending">Pending</option>
              <option value="Finance Approved">Approved</option>
              <option value="Finance Rejected">Rejected</option>
            </select>
          </div>
