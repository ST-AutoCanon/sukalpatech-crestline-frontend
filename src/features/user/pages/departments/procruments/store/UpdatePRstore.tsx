...
export default function SubmittedStoreRequestsPage() {
  ...
  const STORE_STATUS_OPTIONS = ["STORE APPROVED", "STORE REJECTED", "STORE PENDING"];
  ...
  const selectPR = (pr: StorePR) => {
    ...
    setUpdateData({
      ...
      items: pr.items.map((item) => ({ ...item, vendors: item.vendors.map((v) => ({ ...v })) })),
    });
    setNewStatus("");
    setNewComment("");
    setModalOpen(true);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {requests.map((pr) => (
          <div
            key={pr.id}
            onClick={() => selectPR(pr)}
            className="bg-white border border-gray-300 rounded-xl shadow-sm hover:shadow-md transition-all p-4 cursor-pointer flex flex-col min-h-[220px]"
          >
            <h2 className="text-purple-600 font-semibold text-lg mb-2 truncate">PR ID: {pr.id}</h2>
            <div className="flex-1 space-y-1 text-sm">
              <div className="grid grid-cols-1 gap-1">
                <div className="flex justify-between"><span className="text-gray-500">Department</span><span className="font-medium text-gray-800 truncate">{pr.department || "-"}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Priority</span><span className="font-medium text-gray-800 truncate">{pr.priority || "-"}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Required</span><span className="font-medium text-gray-800">{formatDate(pr.required_date)}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Description</span><span className="font-medium text-gray-800 line-clamp-2">{pr.description || "-"}</span></div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex justify-center items-start pt-4 sm:pt-10 z-50 overflow-auto">
          <div className="bg-white w-full max-w-6xl rounded shadow-lg relative flex flex-col max-h-[90vh] overflow-hidden">
            ...
            <div className="overflow-y-auto p-4 sm:p-6 flex-1 space-y-4">
              <h2 className="text-2xl font-bold text-violet-600 mb-2">Update Store</h2>
                          ...
                          return (
                            <div key={vi} className="grid grid-cols-1 sm:grid-cols-8 gap-2 mb-2 min-w-[700px]">
                              ...
                              <div className="flex-1 border rounded px-2 py-1 bg-white text-center min-w-0">
                                {vendor.attachments ? <a href={vendor.attachments} target="_blank" className="text-blue-600 underline">{vendor.attachments.file_name}</a> : ""}
                              </div>
                              {/* Feasibility Comment */}
