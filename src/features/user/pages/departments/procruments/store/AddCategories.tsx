...
const fetchHierarchy = async () => {
  try {
    const res = await api.get(`${API_BASE}/hierarchy`);
  ...
  const fetchRoots = async () => {
    const res = await api.get(`${API_BASE}/root-category`);
    setRoots(res.data || []); 
  };

  const fetchCategories = async (rootId: number) => {
    const res = await api.get(`${API_BASE}/list?root_id=${rootId}`);
    setCategories(res.data || []);
    setProducts([]); setVariants([]); setSubVariants([]);
  };

  const fetchProducts = async (categoryId: number) => {
    const res = await api.get(`${API_BASE}/products?category_id=${categoryId}`);
    setProducts(res.data || []);
    setVariants([]); setSubVariants([]);
  };

  const fetchVariants = async (productId: number) => {
    const res = await api.get(`${API_BASE}/variants?product_id=${productId}`);
    setVariants(res.data || []);
    setSubVariants([]);
  };

  const fetchSubVariants = async (variantId: number) => {
    const res = await api.get(`${API_BASE}/sub-variants?variant_id=${variantId}`);
  ...
  const addRootCategory = async () => {
    ...
    try {
      await api.post(`${API_BASE}/root-category`, { name: rootName });
  ...
  const addCategory = async () => {
    ...
    try {
      await api.post(`${API_BASE}/category`, { name: categoryName, root_category_id: selectedRoot });
  ...
  const addProduct = async () => {
    ...
    try {
      await api.post(`${API_BASE}/product`, { name: productName, category_id: selectedCategory });
  ...
  const addVariant = async () => {
    ...
    try {
      await api.post(`${API_BASE}/variant`, { name: variantName, product_id: selectedProduct });
  ...
  const addSubVariant = async () => {
    ...
    try {
      await api.post(`${API_BASE}/sub-variant`, { name: subVariantName, variant_id: selectedVariant });
  ...
  const handleSearch = async () => {
    ...
    const res = await api.get(`${API_BASE}/search?query=${searchQuery}`);
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-r from-[#4b1b7a] to-[#2d2a8c] p-6 text-black">
      ...
      {/* Tabs */}
<div className="flex gap-6 text-white mb-6 border-b border-white/20 overflow-x-auto whitespace-nowrap">
      ...
      {/* Form Container */}
<div className="bg-white/10 backdrop-blur-md rounded-xl p-4 sm:p-6 w-full max-w-6xl mx-auto">
