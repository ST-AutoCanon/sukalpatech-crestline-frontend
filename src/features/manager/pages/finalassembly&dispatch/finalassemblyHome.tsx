...
import FinalAssemblyDispatch from "./finalassemblypage";
import ProjectsPage from "./projectpage"; // 🔥 adjust path if needed
import UpdateProjectsPage from "./updateprojectpage"; // 🔥 adjust path if needed

const FinalAssemblydispatchHome: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"finalassemblydispatch" | "projects" | "update project">(
    "finalassemblydispatch",
  );
    ...
    <div className="w-full h-full bg-gradient-to-r from-[#4b1b7a] to-[#2d2a8c] text-white">
      <div className="w-full px-4 sm:px-6 md:px-10 pt-5 md:pt-8 flex items-center justify-between">
        ...
        <div className="flex flex-wrap gap-4 sm:gap-4">
          <button
            onClick={() => {
              setActiveTab("finalassemblydispatch");
        ...
        ${
          activeTab === "finalassemblydispatch" && requestType === "drawings"
      ...
      <div className="px-3 sm:px-5 md:px-10 pt-4 md:pt-6 pb-10">
        {activeTab === "finalassemblydispatch" && requestType === "drawings" && (
          <FinalAssemblyDispatch type="drawings" />
        )}
...
export default FinalAssemblydispatchHome;
