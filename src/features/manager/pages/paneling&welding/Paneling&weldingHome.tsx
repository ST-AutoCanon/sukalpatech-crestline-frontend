...
import PannelingWeldingPage from "./Paneling&weldingpage";
import ProjectsPage from "./ProjectsPage"; // 🔥 adjust path if needed
import UpdateProjectsPage from "./UpdateProjects"; // 🔥 adjust path if needed

const PaneelignandWeldignHome: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"panneling&welding" | "projects" | "update project">(
    "panneling&welding",
  );
    ...
    <div className="w-full h-full bg-gradient-to-r from-[#4b1b7a] to-[#2d2a8c] text-white">
      <div className="w-full px-4 sm:px-6 md:px-10 pt-5 md:pt-8 flex items-center justify-between">
        ...
        <div className="flex flex-wrap gap-4 sm:gap-4">
          <button
            onClick={() => {
              setActiveTab("panneling&welding");
        ...
        ${
          activeTab === "panneling&welding" && requestType === "drawings"
      ...
      <div className="px-3 sm:px-5 md:px-10 pt-4 md:pt-6 pb-10">
        {activeTab === "panneling&welding" && requestType === "drawings" && (
          <PannelingWeldingPage type="drawings" />
        )}
...
export default PaneelignandWeldignHome;
