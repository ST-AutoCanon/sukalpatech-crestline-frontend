...
// const HRMSLoginModal = ({ isOpen, onClose, onSwitchToNormalLogin }: any) => {
...
//             <button className="w-full py-3 mt-4 bg-gradient-to-r from-[#0088cc] to-[#6600ff] text-white rounded-xl">
//               Login
//             </button>
//           </div>
...
const HRMSLoginModal: React.FC<HRMSLoginModalProps> = ({
  ...
  // Focus Employee ID input when modal opens
  useEffect(() => {
    if (isOpen && empIdRef.current) empIdRef.current.focus();
  }, [isOpen]);
  ...
  useEffect(() => {
    function onMessage(ev: MessageEvent) {
      ...
      if (msg.type === "login-success") {
        ...
        setTimeout(onClose, 1000); // Give time for user to see message
      } else if (msg.type === "login-failed") {
        setStatusMessage(
          `Login failed${msg.error ? `: ${String(msg.error)}` : ""}`
        );
      } else if (msg.type === "iframe-loaded") {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      ...
      {/* Hidden iframe (can also be external) */}
      <iframe
        id={iframeId}
        title="HRMS Embedded App"
        src={`${childOrigin}`}
        className="hidden"
